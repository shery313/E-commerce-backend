...
from store.serializers import CancelledOrderSerializer,NewsletterSerializer, CartSerializer, CartOrderItemSerializer,ContactSerializer, CouponUsersSerializer, ProductSerializer, TagSerializer ,CategorySerializer, DeliveryCouriersSerializer, CartOrderSerializer, GallerySerializer, BrandSerializer, ProductFaqSerializer, ReviewSerializer,  SpecificationSerializer, CouponSerializer, ColorSerializer, SizeSerializer, AddressSerializer, WishlistSerializer, ConfigSettingsSerializer

# Models
from userauths.models import User
from store.models import CancelledOrder,Newsletter, CartOrderItem, CouponUsers, Cart, Notification,Contact, Product, Tag ,Category, DeliveryCouriers, CartOrder, Gallery, Brand, ProductFaq, Review,  Specification, Coupon, Color, Size, Address, Wishlist
...
class FeaturedProductListView(generics.ListAPIView):
    serializer_class = ProductSerializer
    queryset = Product.objects.filter(status="published", is_featured=True)[:3]
    permission_classes = (AllowAny,)
class SpecialOfferListView(generics.ListAPIView):
    serializer_class = ProductSerializer
    queryset = Product.objects.filter(status="published", special_offer=True)[:3]
    permission_classes = (AllowAny,)
class BestSellerListView(generics.ListAPIView):
    serializer_class = ProductSerializer
    queryset = Product.objects.filter(status="published", is_bestseller=True)[:4]
    permission_classes = (AllowAny,)
class NewArrivalListView(generics.ListAPIView):
    serializer_class = ProductSerializer
    queryset = Product.objects.filter(status="published", is_new_arrival=True)[:7]
    permission_classes = (AllowAny,)
...
class CashOnDeliveryView(generics.CreateAPIView):
    serializer_class = CartOrderSerializer  # No serializer class needed for this view

    def create(self, request, *args, **kwargs):
        order_oid = self.kwargs['order_oid']
        return redirect(settings.SITE_URL + '/order-success/' + order_oid)
...
class ContactCreateView(generics.CreateAPIView):
    ...
    def create(self,request,*args, **kwargs):
        ...
        contact=Contact.objects.create(name=name,email=email,order=order,message=message)
        contact.save()
        return Response({"message":"we have recieved your message "},status=status.HTTP_201_CREATED)
class NewsletterCreateView(generics.CreateAPIView):
    ...
    def create(self,request,*args, **kwargs):
        ...
        newsletter.save()
        return Response({"message":"You have subscribed to our newsletter "},status=status.HTTP_201_CREATED)
